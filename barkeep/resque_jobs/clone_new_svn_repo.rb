...
class CloneNewSvnRepo
  include ResqueJobHelper
  @queue = :clone_new_svn_repo

  # We're willing to spend up to 15 minutes to clone a repo. This can be necessary for giant repos or
  ...
  def self.perform(repo_name, repo_url)
    ...
    repo_path = File.join(REPOS_SVN_ROOT, repo_name)
    ...
    begin
      ...
      Timeout::timeout(CLONE_TIMEOUT) do
        run_shell("cd '#{repo_path}' && svn2git '#{repo_url}' --no-minimize-url")
      end
