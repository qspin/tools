...
class BarkeepServer < Sinatra::Base
  ...
  get "/admin/repos/?" do
    ...
    admin_erb :repos, :locals => {
      ...
      :clone_new_svn_repo_log => tail_log.call("clone_new_svn_repo.log"),
      :fetch_commits_log => tail_log.call("fetch_commits.log"),
      :fetch_svn_commits_log => tail_log.call("fetch_svn_commits.log")
    }
  end
  ...
  # Schedules a SVN repo to be cloned
  #  - url 
  post "/admin/repos/create_new_svn_repo" do
    halt 400, "'url' is required." if (params[:url] || "").strip.empty?
    begin
      add_svn_repo params["url"]
    rescue RuntimeError => e
      halt 400, e.message
    end
    nil
