...
require "resque_jobs/clone_new_svn_repo"
...
module Api
  ...
  def add_svn_repo(url)
    raise "This is not a valid URL." unless Addressable::URI.parse(url)
    repo_name = File.basename(url, ".*")
    repo_path = File.join(REPOS_SVN_ROOT, repo_name)
    raise "There is already a folder named \"#{repo_name}\" in #{REPOS_SVN_ROOT}." if File.exists?(repo_path)
    Resque.enqueue(CloneNewSvnRepo, repo_name, url)
  end
